...
func NewManager(workspace string) *Manager {
	...
	stateFile := filepath.Join(stateDir, "utils.json")
	oldStateFile := filepath.Join(workspace, "utils.json")
...
// SetLastChannel atomically updates the last channel and saves the utils.
...
// SetLastChatID atomically updates the last chat ID and saves the utils.
...
// GetLastChannel returns the last channel from the utils.
...
// GetLastChatID returns the last chat ID from the utils.
...
func (sm *Manager) saveAtomic() error {
	...
	return WriteFileAtomic(sm.stateFile, data, 0o600)
}
