...
import (
	...
	"ok/internal/devices/events"
	"ok/internal/devices/sources"
	"ok/internal/logger"
	"ok/internal/utils"
)
...
func NewService(cfg Config, stateMgr *utils.Manager) *Service {
	s := &Service{
		state:   stateMgr,
		enabled: cfg.Enabled,
...
func (s *Service) SetBus(msgBus *msgbus.MessageBus) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.bus = msgBus
}
...
func (s *Service) sendNotification(ev *events.DeviceEvent) {
	...
	if platform == "" || userID == "" || utils.IsInternalChannel(platform) {
		return
	}
	...
	defer pubCancel()
	msgBus.PublishOutbound(pubCtx, msgbus.OutboundMessage{
		Channel: platform,
		ChatID:  userID,
		Content: msg,
	})
