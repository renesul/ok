...
func NewHeartbeatService(workspace string, intervalMinutes int, enabled bool) *HeartbeatService {
	...
	return &HeartbeatService{
		...
		enabled:   enabled,
		state:     utils.NewManager(workspace),
	}
}

// SetBus sets the message bus for delivering heartbeat results.
func (hs *HeartbeatService) SetBus(msgBus *events.MessageBus) {
	hs.mu.Lock()
	defer hs.mu.Unlock()
	hs.bus = msgBus
}
...
func (hs *HeartbeatService) sendResponse(response string) {
	...
	defer pubCancel()
	msgBus.PublishOutbound(pubCtx, events.OutboundMessage{
		Channel: platform,
		ChatID:  userID,
		Content: response,
	})
...
func (hs *HeartbeatService) parseLastChannel(lastChannel string) (platform, userID string) {
	...
	if utils.IsInternalChannel(platform) {
