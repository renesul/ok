...
func TestRuleClassifier_AttachmentsHardGate(t *testing.T) {
	c := &RuleClassifier{}
	score := c.Score(types.Features{HasAttachments: true})
	if score != 1.0 {
...
func TestRuleClassifier_CodeBlockAlone(t *testing.T) {
	c := &RuleClassifier{}
	score := c.Score(types.Features{CodeBlockCount: 1})
	if score < 0.35 {
...
func TestRuleClassifier_LongMessage(t *testing.T) {
	c := &RuleClassifier{}
	score := c.Score(types.Features{TokenEstimate: 250})
	if score < 0.35 {
...
func TestRuleClassifier_MediumMessage(t *testing.T) {
	c := &RuleClassifier{}
	score := c.Score(types.Features{TokenEstimate: 100})
	if score >= 0.35 {
...
func TestRuleClassifier_ShortMessage(t *testing.T) {
	c := &RuleClassifier{}
	score := c.Score(types.Features{TokenEstimate: 10})
	if score != 0.0 {
...
func TestRuleClassifier_ToolCallDensity(t *testing.T) {
	...
	scoreNone := c.Score(types.Features{RecentToolCalls: 0})
	scoreLow := c.Score(types.Features{RecentToolCalls: 2})
	scoreHigh := c.Score(types.Features{RecentToolCalls: 5})
...
func TestRuleClassifier_DeepConversation(t *testing.T) {
	c := &RuleClassifier{}
	shallow := c.Score(types.Features{ConversationDepth: 5})
	deep := c.Score(types.Features{ConversationDepth: 15})
	if deep <= shallow {
...
func TestRouter_DefaultThreshold(t *testing.T) {
	r := NewRouter(RouterConfig{LightModel: "gemini-flash"})
...
func TestRouter_NegativeThresholdFallsBackToDefault(t *testing.T) {
	r := NewRouter(RouterConfig{LightModel: "gemini-flash", Threshold: -0.1})
	if r.Threshold() != defaultThreshold {
...
func TestRouter_SelectModel_SimpleMessageUsesLight(t *testing.T) {
	r := NewRouter(RouterConfig{LightModel: "gemini-flash", Threshold: 0.35})
	msg := "hi"
...
func TestRouter_SelectModel_CustomThreshold(t *testing.T) {
	r := NewRouter(RouterConfig{LightModel: "gemini-flash", Threshold: 0.05})
	msg := strings.Repeat("word ", 55)
...
func TestRouter_SelectModel_HighThreshold(t *testing.T) {
	r := NewRouter(RouterConfig{LightModel: "gemini-flash", Threshold: 0.99})
	msg := "```go\nfmt.Println()\n```"
	_, usedLight, _ := r.SelectModel(msg, nil, "claude-sonnet-4-6")
	if !usedLight {
...
func TestRouter_LightModel(t *testing.T) {
	r := NewRouter(RouterConfig{LightModel: "my-fast-model", Threshold: 0.35})
	if r.LightModel() != "my-fast-model" {
		t.Errorf("LightModel: got %q, want %q", r.LightModel(), "my-fast-model")
	}
}

// ── NewRouterWithClassifier (internal testing hook) ─────────────────────────

type fixedScoreClassifier struct{ score float64 }

func (f *fixedScoreClassifier) Score(_ types.Features) float64 { return f.score }

func TestRouter_CustomClassifier_LowScore_SelectsLight(t *testing.T) {
	r := NewRouterWithClassifier(
