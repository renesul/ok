...
func (r *RouteResolver) ResolveRoute(input types.RouteInput) types.ResolvedRoute {
	...
	dmScope := types.DMScope(r.cfg.Session.DMScope)
	if dmScope == "" {
	...
	choose := func(agentID string, matchedBy string) types.ResolvedRoute {
...
func (r *RouteResolver) findPeerMatch(bindings []config.AgentBinding, peer *types.RoutePeer) *config.AgentBinding {
