...
func (f *fakeChannel) Send(ctx context.Context, msg events.OutboundMessage) error { return nil }
func (f *fakeChannel) IsRunning() bool                                         { return true }
func (f *fakeChannel) IsAllowed(string) bool                                   { return true }
func (f *fakeChannel) IsAllowedSender(sender events.SenderInfo) bool              { return true }
func (f *fakeChannel) ReasoningChannelID() string                              { return f.id }

func newTestAgentLoop(
	t *testing.T,
) (al *AgentLoop, cfg *config.Config, msgBus *events.MessageBus, provider *mockProvider, cleanup func()) {
	t.Helper()
...
func (h testHelper) executeAndGetResponse(tb testing.TB, ctx context.Context, msg events.InboundMessage) string {
...
func TestProcessMessage_CommandOutcomes(t *testing.T) {
	...
	showResp := helper.executeAndGetResponse(t, context.Background(), events.InboundMessage{
	...
	fooResp := helper.executeAndGetResponse(t, context.Background(), events.InboundMessage{
		Channel:  baseMsg.Channel,
		SenderID: baseMsg.SenderID,
		ChatID:   baseMsg.ChatID,
		Content:  "/foo",
	...
	newResp := helper.executeAndGetResponse(t, context.Background(), events.InboundMessage{
		Channel:  baseMsg.Channel,
		SenderID: baseMsg.SenderID,
		ChatID:   baseMsg.ChatID,
		Content:  "/new",
...
func TestProcessMessage_SwitchModelShowModelConsistency(t *testing.T) {
	...
	switchResp := helper.executeAndGetResponse(t, context.Background(), events.InboundMessage{
		Channel:  "telegram",
		SenderID: "user1",
		ChatID:   "chat1",
...
func TestTargetReasoningChannelID_AllChannels(t *testing.T) {
	...
	al := NewAgentLoop(cfg, events.NewMessageBus(), &mockProvider{})
	chManager, err := channels.NewManager(&config.Config{}, events.NewMessageBus(), nil)
	if err != nil {
...
func TestHandleReasoning(t *testing.T) {
	newLoop := func(t *testing.T) (*AgentLoop, *events.MessageBus) {
		t.Helper()
		tmpDir, err := os.MkdirTemp("", "agent-test-*")
		if err != nil {
			t.Fatalf("Failed to create temp dir: %v", err)
	...
	t.Run("returns promptly when bus is full", func(t *testing.T) {
		...
		for i := 0; ; i++ {
			...
			err := msgBus.PublishOutbound(fillCtx, events.OutboundMessage{
				Channel: "filler",
				ChatID:  "filler",
