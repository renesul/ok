...
type ToolExecOptions = planning.ToolExecOptions

// ToolExecOutput holds the tool result messages to append to the LLM conversation.
type ToolExecOutput = planning.ToolExecOutput
...
func (te *ToolExecutor) ExecuteAll(
	...
	for _, r := range results {
		...
		if !r.result.Silent && r.result.ForUser != "" && opts.SendResponse {
			te.bus.PublishOutbound(ctx, events.OutboundMessage{
		...
		if len(r.result.Media) > 0 {
			parts := make([]events.MediaPart, 0, len(r.result.Media))
			for _, ref := range r.result.Media {
				part := events.MediaPart{Ref: ref}
			...
			te.bus.PublishOutboundMedia(ctx, events.OutboundMediaMessage{
				Channel: opts.Channel,
				ChatID:  opts.ChatID,
				Parts:   parts,
			})
...
func (te *ToolExecutor) makeAsyncCallback(
	...
	return func(_ context.Context, result *tools.ToolResult) {
		...
		if !result.Silent && result.ForUser != "" {
			...
			defer outCancel()
			_ = te.bus.PublishOutbound(outCtx, events.OutboundMessage{
				Channel: opts.Channel,
				ChatID:  opts.ChatID,
		...
		defer pubCancel()
		_ = te.bus.PublishInbound(pubCtx, events.InboundMessage{
			Channel:  "system",
