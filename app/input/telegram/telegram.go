...
func NewTelegramChannel(cfg *config.Config, bus *events.MessageBus) (*TelegramChannel, error) {
...
func (c *TelegramChannel) Send(ctx context.Context, msg events.OutboundMessage) error {
	if !c.IsRunning() {
...
func (c *TelegramChannel) SendMedia(ctx context.Context, msg events.OutboundMediaMessage) error {
	if !c.IsRunning() {
		return channels.ErrNotRunning
	}
