...
type mockChannel struct {
	BaseChannel
	sendFn func(ctx context.Context, msg events.OutboundMessage) error
}

func (m *mockChannel) Send(ctx context.Context, msg events.OutboundMessage) error {
...
func TestSendWithRetry_Success(t *testing.T) {
	...
	var callCount int
	ch := &mockChannel{
		sendFn: func(_ context.Context, _ events.OutboundMessage) error {
			callCount++
			return nil
		},
	}
	...
	msg := events.OutboundMessage{Channel: "test", ChatID: "1", Content: "hello"}
...
func TestSendWithRetry_ContextCancelled(t *testing.T) {
	...
	ch.sendFn = func(_ context.Context, _ events.OutboundMessage) error {
		callCount++
		cancel()
		return fmt.Errorf("timeout: %w", ErrTemporary)
	}
...
func TestWorkerRateLimiter(t *testing.T) {
	...
	w := &channelWorker{
		ch:      ch,
		queue:   make(chan events.OutboundMessage, 10),
	...
	// Enqueue 4 messages
	for i := range 4 {
		w.queue <- events.OutboundMessage{Channel: "test", ChatID: "1", Content: fmt.Sprintf("msg%d", i)}
	}
...
func TestRunWorker_MessageSplitting(t *testing.T) {
	...
	ch := &mockChannelWithLength{
		mockChannel: mockChannel{
			sendFn: func(_ context.Context, msg events.OutboundMessage) error {
				mu.Lock()
	...
	w.queue <- events.OutboundMessage{Channel: "test", ChatID: "1", Content: "hello world"}
...
func TestPreSend_PlaceholderEditSuccess(t *testing.T) {
	...
	msg := events.OutboundMessage{Channel: "test", ChatID: "123", Content: "hello"}
...
func TestPreSendStillWorksWithWrappedTypes(t *testing.T) {
	...
	msg := events.OutboundMessage{Channel: "test", ChatID: "chat1", Content: "response"}
	edited := m.preSend(context.Background(), "test", msg, ch)
