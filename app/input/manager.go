...
type channelWorker struct {
	ch         Channel
	queue      chan events.OutboundMessage
	mediaQueue chan events.OutboundMediaMessage
	done       chan struct{}
...
func (m *Manager) preSend(ctx context.Context, name string, msg events.OutboundMessage, ch Channel) bool {
	key := name + ":" + msg.ChatID
...
func NewManager(cfg *config.Config, messageBus *events.MessageBus, store media.MediaStore) (*Manager, error) {
	m := &Manager{
...
func newChannelWorker(name string, ch Channel) *channelWorker {
	...
	return &channelWorker{
		ch:         ch,
		queue:      make(chan events.OutboundMessage, defaultChannelQueueSize),
		mediaQueue: make(chan events.OutboundMediaMessage, defaultChannelQueueSize),
...
func (m *Manager) sendWithRetry(ctx context.Context, name string, w *channelWorker, msg events.OutboundMessage) {
...
func dispatchLoop[M any](
	...
	for {
		...
		if utils.IsInternalChannel(channel) {
			continue
		}
...
func (m *Manager) dispatchOutbound(ctx context.Context) {
	dispatchLoop(
		...
		func(msg events.OutboundMessage) string { return msg.Channel },
		func(ctx context.Context, w *channelWorker, msg events.OutboundMessage) bool {
			select {
			case w.queue <- msg:
				return true
			case <-ctx.Done():
...
func (m *Manager) dispatchOutboundMedia(ctx context.Context) {
	dispatchLoop(
		...
		func(msg events.OutboundMediaMessage) string { return msg.Channel },
		func(ctx context.Context, w *channelWorker, msg events.OutboundMediaMessage) bool {
			select {
...
func (m *Manager) sendMediaWithRetry(ctx context.Context, name string, w *channelWorker, msg events.OutboundMediaMessage) {
