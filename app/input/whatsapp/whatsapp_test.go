...
func TestIsAllowedMessage(t *testing.T) {
	...
	t.Run("allowed_groups filters group messages", func(t *testing.T) {
		...
		sender := msgbus.SenderInfo{Platform: "whatsapp", PlatformID: "5511888888888@s.whatsapp.net"}

		// Allowed group
		if !ch.isAllowedMessage(types.NewJID("120363012345", types.GroupServer), sender.PlatformID, "120363012345@g.us", sender, false) {
			t.Error("should allow message from allowed group")
		}
		// Disallowed group
		if ch.isAllowedMessage(types.NewJID("999999999", types.GroupServer), sender.PlatformID, "999999999@g.us", sender, false) {
	...
	t.Run("allowed_contacts filters direct messages", func(t *testing.T) {
		...
		sender := msgbus.SenderInfo{Platform: "whatsapp", PlatformID: "5511999999999@s.whatsapp.net"}

		// Allowed contact
		if !ch.isAllowedMessage(types.NewJID("5511999999999", types.DefaultUserServer), sender.PlatformID, "5511999999999@s.whatsapp.net", sender, false) {
			t.Error("should allow DM from allowed contact")
		}
		// Disallowed contact
		sender2 := msgbus.SenderInfo{Platform: "whatsapp", PlatformID: "5511888888888@s.whatsapp.net"}
		if ch.isAllowedMessage(types.NewJID("5511888888888", types.DefaultUserServer), sender2.PlatformID, "5511888888888@s.whatsapp.net", sender2, false) {
	...
	t.Run("empty lists reject everything", func(t *testing.T) {
		...
		sender := msgbus.SenderInfo{Platform: "whatsapp", PlatformID: "anyone"}
		if ch.isAllowedMessage(types.NewJID("anyone", types.DefaultUserServer), sender.PlatformID, "anyone@s.whatsapp.net", sender, false) {
			t.Error("should reject DMs when allowed_contacts is empty")
		}
		if ch.isAllowedMessage(types.NewJID("somegroup", types.GroupServer), sender.PlatformID, "somegroup@g.us", sender, false) {
