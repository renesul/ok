...
type Channel interface {
	...
	Send(ctx context.Context, msg events.OutboundMessage) error
	IsRunning() bool
	IsAllowed(senderID string) bool
	IsAllowedSender(sender events.SenderInfo) bool
...
func (c *BaseChannel) IsAllowedSender(sender events.SenderInfo) bool {
	if len(c.allowList) == 0 {
		return true
	}
