...
func (c *DiscordChannel) Send(ctx context.Context, msg events.OutboundMessage) error {
	if !c.IsRunning() {
...
func (c *DiscordChannel) SendMedia(ctx context.Context, msg events.OutboundMediaMessage) error {
	if !c.IsRunning() {
		return channels.ErrNotRunning
	}
