...
func NewSlackChannel(cfg config.SlackConfig, messageBus *events.MessageBus) (*SlackChannel, error) {
...
func (c *SlackChannel) Send(ctx context.Context, msg events.OutboundMessage) error {
	if !c.IsRunning() {
...
func (c *SlackChannel) SendMedia(ctx context.Context, msg events.OutboundMediaMessage) error {
	if !c.IsRunning() {
		return channels.ErrNotRunning
	}
...
func (c *SlackChannel) handleMessageEvent(ev *slackevents.MessageEvent) {
	...
	if ev.Message != nil && len(ev.Message.Files) > 0 {
		for _, file := range ev.Message.Files {
			...
			if utils.IsAudioFile(file.Name, file.Filetype) {
				content += "\n[audio]"
			} else {
				content += fmt.Sprintf("\n[file: %s]", file.Name)
			}
	...
	peer := events.Peer{Kind: peerKind, ID: peerID}
...
func (c *SlackChannel) handleAppMention(ev *slackevents.AppMentionEvent) {
	...
	if !c.IsAllowedSender(events.SenderInfo{
		Platform:    "slack",
		PlatformID:  ev.User,
		CanonicalID: identity.BuildCanonicalID("slack", ev.User),
	}) {
	...
	senderID := ev.User
	mentionSender := events.SenderInfo{
		Platform:    "slack",
		PlatformID:  senderID,
	...
	mentionPeer := events.Peer{Kind: mentionPeerKind, ID: mentionPeerID}
...
func (c *SlackChannel) handleSlashCommand(event socketmode.Event) {
	...
	c.HandleMessage(
		c.ctx,
		events.Peer{Kind: "channel", ID: channelID},
		"",
		senderID,
		chatID,
		content,
