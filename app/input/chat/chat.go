...
// HistoryCapable is implemented by channels that support session history injection.
type HistoryCapable interface {
	SetHistoryProvider(hp HistoryProvider)
}

// ChatChannel is the built-in web chat channel.
...
func NewChatChannel(cfg *config.Config, messageBus *events.MessageBus) *ChatChannel {
	base := channels.NewBaseChannel("chat", nil, messageBus, nil)
	return &ChatChannel{
		BaseChannel: base,
		upgrader: websocket.Upgrader{
...
func (c *ChatChannel) Send(ctx context.Context, msg events.OutboundMessage) error {
	if !c.IsRunning() {
...
func (c *ChatChannel) handleInbound(cc *chatConn, msg chatMsg) {
	...
	peer := events.Peer{Kind: "direct", ID: chatID}
