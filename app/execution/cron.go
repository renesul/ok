...
func NewCronTool(
	cronService *cron.CronService, executor JobExecutor, msgBus *events.MessageBus, workspace string, restrict bool,
...
func (t *CronTool) ExecuteJob(ctx context.Context, job *cron.CronJob) string {
	...
	if job.Payload.Command != "" {
		...
		defer pubCancel()
		t.msgBus.PublishOutbound(pubCtx, events.OutboundMessage{
			Channel: channel,
			ChatID:  chatID,
			Content: output,
		})
