...
func (s *InteractionStore) flushLocked() error {
	...
	if err := fileutil.WriteFileAtomic(filepath.Join(s.dir, "messages.json"), msgsData, 0o600); err != nil {
		return err
	}

	// Write embeddings.bin
	embData := encodeEmbeddings(s.embeddings)
	if err := fileutil.WriteFileAtomic(filepath.Join(s.dir, "embeddings.bin"), embData, 0o600); err != nil {
		return err
	}
	...
	return fileutil.WriteFileAtomic(filepath.Join(s.dir, "metadata.json"), metaData, 0o600)
}
